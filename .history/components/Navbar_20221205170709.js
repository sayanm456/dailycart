    ...
    <div className="flex flex-col md:flex-row md:justify-start justify-between items-center mb-1 my-4 shadow-md sticky top-0 bg-white z-10">
      ...
      <div className="nav">
        <ul className="flex items-center space-x-4 font-bold md:text-md">
          <Link href={"/tshirts"}>
            <a>
              <li className="hover:text-violet-500">Tshirts</li>
            </a>
          </Link>
          <Link href={"/hoodies"}>
            <a>
      ...
      <div className="cart items-center absolute right-0 top-2 mx-5 cursor-pointer flex">
       ...
       {dropdown && <div onMouseOver={()=>{setDropdown(true)}} onMouseLeave={()=>{setDropdown(false)}} className="absolute right-8 bg-violet-400 top-6 rounded-md px-5 w-32">
        ...
        {!user.value && <Link href={'\login'}><a>
           {/* <button className='bg-violet-600 px-2 py-1 hover:bg-transparent hover:bg-white-500 hover:text-violet-500 rounded-md text-sm text-white mx-2'>Login</button> */}
           <button type="button" className="font-cabin bg-transparent hover:bg-violet-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-violet-500 hover:border-transparent rounded-md mx-10">Login</button>
           </a></Link>}
